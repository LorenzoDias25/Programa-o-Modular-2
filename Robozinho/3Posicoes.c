...
main() {
  ...
  system("clear");
  printf("Menu posição B\n");
  printf("Regras: \n");
  printf("A posição B não pode ser igual a posição A\n");
  printf("Posição A: %d,%d\n", pil, pic);
  printf("A linha não pode ser menor que 0 e maior que %d\n", nl - 1);
  printf("A coluna não pode ser menor que 0 e maior que %d\n", nc - 1);
  printf("Obstáculos: \n");
  for (temp = 0; temp < nb; temp++) {
  ...
  do {
    ...
    auxcb = 0;
    for (temp = 0; temp < nb; temp++) {
      if ((pbl == vetorl[temp]) && (pbc == vetorc[temp])) {
  ...
  } while (((auxlb != nb) && (auxcb != nb)) || ((pbl == pil) && pbc == pic));
  system("clear");
  printf("Menu posição C\n");
  printf("Regras: \n");
  printf("A posição C não pode ser igual a posição A ou B\n");
  printf("Posição A: %d,%d\n", pil, pic);
  printf("Posição B: %d,%d\n", pbl, pbc);
  printf("A linha não pode ser menor que 0 e maior que %d\n", nl - 1);
  printf("A coluna não pode ser menor que 0 e maior que %d\n", nc - 1);
  printf("Obstáculos: \n");
  for (temp = 0; temp < nb; temp++) {
  ...
  do {
    ...
    if (((pfl == pil && pfc == pic)) || ((pbl == pfl && pbc == pfc))) {
      printf("Posição inválida, digite novamente \n");
      lb = 1;
    } else {
      lb = 0;
  ...
  auxl = pil;
  auxc = pic;
  int auxbl = pbl, auxbc = pbc;
  int auxcl = pfl, auxcc = pfc;
  pfl = pbl;
  pfc = pbc;
  do {
    if (pil < pfl) {
    ...
    if (((pil != pfl) || (pic != pfc)) || (timer == (nl * nl * nc))) {
  ...
  matriz[auxl][auxc] = 'A';
  matriz[auxbl][auxbc] = 'B';
  matriz[auxcl][auxcc] = 'C';
  // printf("Timer: %d\n", timer);
  for (l = 0; l < nl; l++) {
    for (c = 0; c < nc; c++) {
      printf("%c - ", matriz[l][c]);
